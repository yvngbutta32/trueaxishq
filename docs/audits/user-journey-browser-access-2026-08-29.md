...
After the session-cookie clearing compatibility adjustment, the controlled active staff session still rendered the Field workspace and its Sign out control. Selecting it again returned the browser to the ordinary local sign-in boundary with the local confirmation. The affected server log no longer produced the observed `clearCookie` lifetime-option deprecation during this recheck. This does not establish other browser versions, external authentication providers, or production behavior.

For the pending owner-navigation recheck, the controlled local owner sign-in reached the owner-only Admin workspace through the supported local form. No password or account identifier is retained in this record. The next check is limited to the Dashboard’s Job Photos navigation and will not alter owner, client, job, photo, invite, or staff records.

On the pre-repair local Dashboard, selecting the visibly labeled Job Photos sidebar control opened the owner Admin route instead. Browser geometry inspection found the Job Photos element below the visible navigation boundary and overlapped by the pinned Admin footer control, even though its source mapping targeted the photo panel. The repair moved Job Photos directly beneath Jobs, above the crowded lower navigation boundary. In the controlled owner browser, selecting Job Photos then rendered the Job Photos workspace. The adjacent Settings control rendered Settings, and the separate Admin control opened the owner Admin workspace. No photo, job, client, staff, or financial record was changed during this check.

The controlled owner Settings view previously produced a global client query error when the optional Google Calendar client configuration was absent. After the recovery adjustment, the protected authorization query returned a setup-required state instead. The owner view rendered “Owner setup is required before Google authorization can begin” with a disabled “Setup required” control, and the browser console did not show the earlier global query error in that check. No provider authorization, calendar connection, event synchronization, credential configuration, or production behavior was attempted or established.

The same review identified two further state-integrity refinements. Owner Calendar copy now distinguishes a connected authorization record with synchronization enabled from one with synchronization paused. The protected synchronization toggle now requires its final owner-scoped authorization-record update to affect exactly one record; a missing record returns a setup precondition rather than a false success. These changes are covered by focused local tests. No connected or paused provider account, remote Calendar state, synchronization event, or production result was exercised.
