...
### Selected field-operations refinement: private route-order suggestion
- [x] Add an owner-triggered private map route-order suggestion that requires manual review before changing the session-only stop sequence.
...
### Selected credential-free milestone: Google Calendar connection removal
- [x] Verify the existing owner-scoped control removes the locally stored Google Calendar connection record without representing local removal as remote provider revocation.
...
### Selected private foundation: customer-asset lifecycle
...
### Selected private planning correction: inactive-job generation protection
- [x] Prevent a recurring plan for a completed or cancelled owned job from generating a future internal service visit while retaining all history.
...
- [x] Add focused update, overlap, scope, and private-projection coverage; validate and document the refinement.

### Selected field-operations correction: cancelled-visit availability state
...
### Observed booking-message correction: delivery-marker accuracy
...
### Observed session-cookie clearing compatibility
...
### Observed development service-worker cache recovery
- [x] Prevent local development service-worker registration from serving stale client bundles during owner, staff, and portal validation while retaining production installable-app registration intent.
- [ ] Add focused registration-environment coverage, verify the local controlled browser loads the current bundle without manual service-worker removal, and document production-installation limits.
