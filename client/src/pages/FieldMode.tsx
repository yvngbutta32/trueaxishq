...
export default function FieldMode() {
  ...
  useEffect(() => {
    if (!jobId) return;
    try { setNote(localStorage.getItem(fieldModeDraftKey(jobId)) ?? ""); } catch { setNote(""); }
  }, [jobId]);
  ...
  const postUpdate = trpc.jobs.addUpdate.useMutation({
    onSuccess: () => {
      setNote("");
      if (jobId) try { localStorage.removeItem(fieldModeDraftKey(jobId)); } catch { /* draft persistence is best effort */ }
  ...
  const persistDraft = (value: string) => {
    ...
    if (!jobId) return;
    try {
      if (value.trim()) localStorage.setItem(fieldModeDraftKey(jobId), value);
      else localStorage.removeItem(fieldModeDraftKey(jobId));
  ...
  return <div className="mx-auto max-w-3xl space-y-5 pb-8">
    ...
    {activeJob && detail.data ? <>
      ...
      <section className="rounded-2xl border border-[rgba(26,26,26,0.1)] bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-bold text-[#1A1A1A]">Send a client update</h3><p className="text-xs text-[rgba(26,26,26,0.62)]">Keep progress visible without switching apps.</p></div><Clock3 className="h-5 w-5 text-[#D4922A]" /></div><textarea value={note} onChange={event => persistDraft(event.target.value)} maxLength={5000} rows={3} placeholder="Work completed, arrival details, or next step…" className="mt-4 w-full resize-none rounded-xl border border-[rgba(26,26,26,0.14)] bg-white p-3 text-sm text-[#1A1A1A] outline-none focus:ring-2 focus:ring-[#D4922A]/35" /><div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-[rgba(26,26,26,0.62)]">{note.trim() ? "Draft saved on this device." : ""}</p><Button disabled={!isOnline || !note.trim() || postUpdate.isPending} onClick={() => postUpdate.mutate({ jobId: activeJob.id, message: note.trim(), visibleToClient: true })} className="min-h-11 bg-[#1C2333] text-white hover:bg-[#2B3446]">{postUpdate.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />} Post update</Button></div></section>
