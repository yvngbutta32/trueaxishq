...
import { fieldModeDraftKey, getFieldModeConnectivityMessage, isRetryableFieldError, readFieldModeStatusQueue, writeFieldModeStatusQueue, type FieldModeQueuedStatus } from "@shared/fieldModeRecovery";
import { chooseFieldModeUpdateTemplate, fieldModeUpdateTemplates } from "@shared/fieldModeClientUpdates";
import { Camera, CheckCircle2, Circle, Clock3, CloudOff, Loader2, Play, RotateCw, Send, Square, Timer, Upload } from "lucide-react";
...
export default function FieldMode() {
  ...
  const [queuedStatuses, setQueuedStatuses] = useState<FieldModeQueuedStatus[]>([]);
  ...
  useEffect(() => {
    try { setQueuedStatuses(readFieldModeStatusQueue(localStorage)); } catch { setQueuedStatuses([]); }
  }, []);
  ...
  useEffect(() => {
    if (!jobId) return;
    try { setNote(sessionStorage.getItem(fieldModeDraftKey(jobId)) ?? ""); } catch { setNote(""); }
  }, [jobId]);
  ...
  useEffect(() => {
    if (!isOnline || !queuedStatuses.length || updateJobStatus.isPending) return;
    const [next] = queuedStatuses;
    updateJobStatus.mutate({ id: next.jobId, status: next.status, expectedStatus: next.expectedStatus }, {
      onSuccess: () => {
        const remaining = queuedStatuses.slice(1);
        setQueuedStatuses(remaining);
        try { writeFieldModeStatusQueue(localStorage, remaining); } catch { /* queue remains in memory */ }
        void utils.jobs.list.invalidate();
        if (jobId === next.jobId) void utils.jobs.get.invalidate({ id: next.jobId });
        toast.success("Queued field status synced.");
      },
      onError: error => {
  ...
  }, [isOnline, queuedStatuses, updateJobStatus.isPending, jobId, utils.jobs.get, utils.jobs.list]);
  const postUpdate = trpc.jobs.addUpdate.useMutation({
    onSuccess: () => {
      setNote("");
      if (jobId) try { sessionStorage.removeItem(fieldModeDraftKey(jobId)); } catch { /* draft persistence is best effort */ }
  ...
  const persistDraft = (value: string) => {
    ...
    if (!jobId) return;
    try {
      if (value.trim()) sessionStorage.setItem(fieldModeDraftKey(jobId), value);
      else sessionStorage.removeItem(fieldModeDraftKey(jobId));
  ...
  const captureProof = async (file?: File) => {
    ...
    if (!isOnline) { setRetryPhoto(file); toast.error("Photo is ready to retry when your connection returns."); return; }
    ...
    try {
      ...
      if (isRetryableFieldError(error)) {
        setRetryPhoto(file);
        toast.error("Proof upload paused. Retry when the connection is stable.");
  ...
  const requestStatusChange = (status: "in_progress" | "completed") => {
    if (!activeJob) return;
    if (!isOnline) {
      const next = [...queuedStatuses.filter(item => item.jobId !== activeJob.id), { jobId: activeJob.id, status, expectedStatus: activeJob.status as FieldModeQueuedStatus["expectedStatus"], queuedAt: new Date().toISOString() }];
      setQueuedStatuses(next);
      try { writeFieldModeStatusQueue(localStorage, next); } catch { toast.error("Unable to save the offline status queue on this device."); return; }
      toast.success("Status saved on this device and will sync when you reconnect.");
      return;
    }
    updateJobStatus.mutate({ id: activeJob.id, status, expectedStatus: activeJob.status as FieldModeQueuedStatus["expectedStatus"] });
  };
  ...
  return <div className="mx-auto max-w-3xl space-y-5 pb-8">
    ...
    {offlineMessage && <section role="status" className="flex gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><CloudOff className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><p>{offlineMessage}</p></section>}
    ...
    {activeJob && detail.data ? <>
      <section className="rounded-2xl bg-[#1C2333] p-5 text-white shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-wide text-[#f5c36d]">{activeJob.jobNumber}</p><h2 className="mt-1 text-xl font-bold">{activeJob.title}</h2><p className="mt-1 text-sm text-white/70">{detail.data.client?.name ?? "Client"} · {completion}% checklist complete</p></div><span className="rounded-full bg-white/10 px-2.5 py-1 text-xs font-semibold capitalize">{activeJob.status.replaceAll("_", " ")}</span></div>
        <div className="mt-4 flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-white/5 p-3"><p className="mr-auto text-xs text-white/75">Update the internal job stage deliberately. This does not send a client notification.{queuedStatuses.some(item => item.jobId === activeJob.id) ? " Pending sync saved on this device." : ""}</p>{["lead", "quoted", "approved", "scheduled"].includes(activeJob.status) && <Button type="button" size="sm" onClick={() => requestStatusChange("in_progress")} disabled={updateJobStatus.isPending} className="min-h-10 bg-[#D4922A] text-white hover:bg-[#b87814]">{updateJobStatus.isPending ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Play className="mr-1 h-4 w-4" />} Start on-site work</Button>}{activeJob.status === "in_progress" && <Button type="button" size="sm" variant="outline" onClick={() => { if (window.confirm("Mark this job complete? You can still send a separate client update afterward.")) requestStatusChange("completed"); }} disabled={updateJobStatus.isPending} className="min-h-10 border-white/30 bg-white/10 text-white hover:bg-white/20">{updateJobStatus.isPending ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-1 h-4 w-4" />} Mark complete</Button>}</div>
