...
import { AlertTriangle, ArrowDown, ArrowUp, CalendarOff, CalendarPlus, CarFront, CheckCircle2, Clock3, Loader2, MapPin, Navigation, Route, X } from "lucide-react";
...
export default function DispatchBoard() {
  ...
  const [clientVisitUpdateEdit, setClientVisitUpdateEdit] = useState<ClientVisitUpdateEdit | null>(null);
  ...
  const correctClientVisitUpdate = trpc.dispatch.updateVisit.useMutation({
    onSuccess: () => { invalidate(); setClientVisitUpdateEdit(null); toast.success("Client-visible visit update saved."); },
...
const startTracking = trpc.tracking.start.useMutation({
  onSuccess: async (d: { trackUrl: string }) => {
    utils.dispatch.listVisits.invalidate();
    try {
      await navigator.clipboard.writeText(`${window.location.origin}${d.trackUrl}`);
      toast.success("Live tracking started — link copied. Share it with your client. It ends automatically when the visit arrives.");
    } catch {
      toast.success(`Live tracking started. Share this link: ${d.trackUrl}`);
    }
  },
  onError: (e: { message: string }) => toast.error(e.message),
});
const stopTracking = trpc.tracking.stop.useMutation({
  onSuccess: (d: { stopped: boolean }) => {
    utils.dispatch.listVisits.invalidate();
    if (d.stopped) toast.success("Live tracking stopped.");
    else toast.message("Tracking had already ended (the link auto-revokes on arrival).");
  },
  onError: (e: { message: string }) => toast.error(e.message),
});
  ...
  const submitClientVisitUpdate = () => {
    if (!clientVisitUpdateEdit) return;
    correctClientVisitUpdate.mutate({
      id: clientVisitUpdateEdit.id,
      clientVisible: clientVisitUpdateEdit.clientVisible,
      clientUpdate: clientVisitUpdateEdit.clientVisible ? clientVisitUpdateEdit.clientUpdate.trim() || null : null,
    });
  };
  ...
  <button type="button" onClick={() => startTracking.mutate({ visitId: visit.id })} disabled={startTracking.isPending} className="rounded-lg border border-[#D4922A]/40 px-2 py-2 text-xs font-semibold text-[#B87716] hover:bg-amber-50 disabled:opacity-60" aria-label={`Share live tracking for ${visit.title}`} title="Starts a consent-based live link for this visit only. It flips the visit to en route, copies the link, and ends automatically on arrival."><Navigation className="mr-1 inline h-3.5 w-3.5" />Share live tracking</button>
    ...
    <section className="rounded-2xl border border-teal-200 bg-teal-50/40 p-5"><div><h2 className="font-bold text-teal-950">Correct client visit update</h2><p className="mt-1 max-w-2xl text-xs leading-relaxed text-teal-900/75">Choose the exact client-facing update for an active service visit. Staff assignment, dispatch notes, capacity, availability, routes, GPS, and internal history remain private.</p></div>{activeVisits.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-teal-200 bg-white/70 px-4 py-3 text-sm text-teal-950/75">There are no active service visits available for a client-update correction.</p> : <div className="mt-4 space-y-3"><label className="block max-w-xl text-sm font-semibold text-[#1A1A1A]">Service visit to curate<select value={clientVisitUpdateEdit?.id ?? ""} onChange={event => { const visit = activeVisits.find(item => item.id === Number(event.target.value)); setClientVisitUpdateEdit(visit ? { id: visit.id, title: visit.title, clientVisible: visit.clientVisible, clientUpdate: visit.clientUpdate ?? "" } : null); }} className="mt-1.5 block w-full rounded-lg border border-teal-200 bg-white px-3 py-2 text-sm font-normal text-[#1A1A1A] outline-none focus:ring-2 focus:ring-teal-300"><option value="">Choose an active private visit…</option>{activeVisits.map(visit => <option key={visit.id} value={visit.id}>{visit.title} · {dateTime(visit.scheduledStart)}</option>)}</select></label>{clientVisitUpdateEdit && <div className="max-w-xl space-y-3 rounded-xl border border-teal-200 bg-white p-4"><label className="flex items-start gap-2 rounded-lg border border-teal-200 bg-teal-50 p-3 text-xs text-teal-950"><input type="checkbox" checked={clientVisitUpdateEdit.clientVisible} onChange={event => setClientVisitUpdateEdit(current => current ? { ...current, clientVisible: event.target.checked } : current)} className="mt-0.5 h-4 w-4 accent-[#007A68]" /><span><strong>Show this curated visit in the client portal.</strong> Turn it off to remove this visit’s client-facing update.</span></label><label className="block text-sm font-semibold text-[#1A1A1A]">Client-facing update<textarea value={clientVisitUpdateEdit.clientUpdate} disabled={!clientVisitUpdateEdit.clientVisible} onChange={event => setClientVisitUpdateEdit(current => current ? { ...current, clientUpdate: event.target.value } : current)} maxLength={500} rows={3} placeholder="e.g. Your appointment window is confirmed." className="mt-1.5 block w-full resize-y rounded-lg border border-teal-200 bg-white px-3 py-2 text-sm font-normal text-[#1A1A1A] outline-none focus:ring-2 focus:ring-teal-300 disabled:bg-slate-50" /><span className="mt-1 block text-xs font-normal text-[rgba(26,26,26,0.62)]">Use concise client-safe language. This stores the portal update; it does not send email, SMS, push, calendar, or any other delivery.</span></label><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => setClientVisitUpdateEdit(null)}>Cancel correction</Button><Button type="button" onClick={submitClientVisitUpdate} disabled={correctClientVisitUpdate.isPending} className="bg-teal-700 text-white hover:bg-teal-800">{correctClientVisitUpdate.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save client-visible update"}</Button></div></div>}</div>}</section>
