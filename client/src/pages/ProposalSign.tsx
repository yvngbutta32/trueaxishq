...
export default function ProposalSign() {
  ...
  const handleSign = () => {
    ...
    if (proposalPackages.length && !effectiveSelectedPackageId) {
      toast.error("Choose one proposal option before signing.");
      return;
    }
    signMutation.mutate({ token, signatureName: signatureName.trim(), selectedPackageId: effectiveSelectedPackageId ?? undefined });
  };
  ...
  const recommendedPackageId = proposalPackages.find(option => option.recommended === true)?.id ?? null;
  const effectiveSelectedPackageId = selectedPackageId ?? recommendedPackageId;
    ...
    <div className="min-h-screen bg-[#F2F0EC] text-[#1A1A1A]">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="p-5 rounded-2xl bg-white border border-[#D4922A]/25 space-y-4">
          ...
          <Button
            onClick={handleSign}
            disabled={signMutation.isPending || !signatureName.trim() || !agreementChecked || (proposalPackages.length > 0 && !effectiveSelectedPackageId)}
            className="w-full py-3 rounded-xl font-bold text-sm gap-2 transition-all"
            style={{
              background: signatureName.trim() && agreementChecked && (!proposalPackages.length || effectiveSelectedPackageId) ? "linear-gradient(135deg, #D4922A, #F0A830)" : undefined,
              opacity: signatureName.trim() && agreementChecked && (!proposalPackages.length || effectiveSelectedPackageId) ? 1 : 0.5,
            }}
          >
