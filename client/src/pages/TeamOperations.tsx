...
export default function TeamOperations() {
  ...
  const operationsExceptions = useMemo(() => buildOperationsExceptions({
    ...
    visits: visits.map(visit => ({ id: visit.id, teamMemberId: visit.teamMemberId, teamMemberName: visit.teamMemberName, title: visit.title, scheduledStart: new Date(visit.scheduledStart), scheduledEnd: new Date(visit.scheduledEnd), status: visit.status })),
    ...
    <section className="rounded-2xl border border-[rgba(26,26,26,0.1)] bg-white p-5">
      ...
      {capacity.length === 0 ? <EmptyRoster onAdd={() => setMemberDialogOpen(true)} /> : <div className="mt-5 grid gap-3 xl:grid-cols-2">{capacity.map(member => {
        ...
        return <article key={member.id} className={`rounded-xl border p-4 ${member.active ? "border-[rgba(26,26,26,0.1)]" : "border-slate-100 bg-slate-50 opacity-75"}`}>
          ...
          <div className="mt-4 flex items-end justify-between gap-3"><div><p className={`text-lg font-bold ${member.overCapacity ? "text-rose-600" : "text-[#1A1A1A]"}`}>{hours(member.plannedMinutes)} <span className="text-xs font-medium text-[rgba(26,26,26,0.5)]">of {hours(member.weeklyCapacityMinutes)}</span></p><p className="mt-0.5 text-xs text-[rgba(26,26,26,0.55)]">{member.overCapacity ? `${hours(member.plannedMinutes - member.weeklyCapacityMinutes)} over capacity` : `${hours(member.remainingMinutes)} available`}</p></div><span className={`rounded-full px-2 py-1 text-xs font-bold ${member.overCapacity ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}`}>{member.overCapacity ? "Review load" : "On track"}</span></div>
